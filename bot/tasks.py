...
def eod_message(latest: dict, st: dict, ev: dict, summ: dict, store: dict) -> str:
    ...
    if sh and sh.get("equity"):
        out.append(f"Shadow account (every proposal taken): {rp(sh['equity'])} ({sh['equity'] / start - 1:+.2%}).")
    if ev["filled"]:
...
def cmd_eod(a) -> None:
    ...
    store = dict(session=session, asof=latest["asof"], created_utc=now_utc(), mode=m, gate=gate.state["state"],
                 ...
                 shadow=dict(equity=shst["history"][-1]["equity"] if shst["history"] else None,
    ...
    if new_day or a.force:
        ...
        print("No new session since the last run (holiday, or prices not published yet). Nothing sent.")
