...
def download(start: str = "2014-01-01") -> pd.DataFrame:
    ...
    extra = [x.strip().upper() for x in os.environ.get("EXTRA_TICKERS", "").split(",") if x.strip()]
    tickers = [s + ".JK" for s in list(UNIVERSE) + [x for x in extra if x not in UNIVERSE]] + [INDEX]
