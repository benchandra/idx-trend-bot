...
def build_orders(latest: dict, st: dict) -> list[dict]:
    ...
    equity = st["cash"] + sum(p["shares"] * (stocks[k]["close"] if k in stocks and p.get("managed", True) else p.get("last") or 0)
                              for k, p in st["positions"].items())
    orders, stopped = [], set()
    for sym, p in st["positions"].items():
        s = stocks.get(sym)
        if not s or p["shares"] <= 0 or not p.get("managed", True):
            continue
        why = None
        if not s["sma200_ok"]:
...
def process_day(st: dict, raw: pd.DataFrame, latest: dict) -> tuple[dict, dict, bool]:
    ...
    for sym, pos in st["positions"].items():
        if sym in stocks and pos.get("managed", True):
            c = stocks[sym]["close"]
        else:  # legacy holding outside the bot's universe (or marked unmanaged): value it, never trade it
            bar = _bar(raw, sym, day)
            c = bar["Close"] if bar else pos.get("last") or pos["cost"] / max(pos["shares"], 1)
        pos["peak"] = max(pos.get("peak", c), c)
        pos["last"] = c
        value += pos["shares"] * c
    ...
    for sym, pos in st["positions"].items():
        s = stocks.get(sym)
        if s is None or not pos.get("managed", True):
            continue
